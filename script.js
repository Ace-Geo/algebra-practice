...
function render(forcedStatus) {
    ...
    for(let r=0; r<8; r++) for(let c=0; c<8; c++) {
        ...
        if(hints.some(h => h.r===r && h.c===c)) {
            const dot = document.createElement('div'); dot.className = piece==='' ? 'hint-dot' : 'hint-capture'; sq.appendChild(dot);
        }
        if(piece) {
            const span = document.createElement('span'); span.className = `piece ${isWhite(piece)?'w-piece':'b-piece'}`; span.textContent = piece; sq.appendChild(span);
        }
